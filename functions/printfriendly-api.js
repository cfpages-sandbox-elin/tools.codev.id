...
export async function onRequest({ request, env }) { // Add env if you intend to use env vars later
  ...
  if (!apiKey) {
    ...
    // CORS headers for error response as well
  ...
  try {
    ...
    console.error("Error in printfriendly-api Function:", error);
