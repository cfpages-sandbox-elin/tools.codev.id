...
import { logToConsole, fetchAndParseSitemap } from './article-helpers.js'; // Moved fetchAndParseSitemap call here
import {
...
// Removed fetchAndParseSitemap import from helpers as it's called here now
...
function initializeApp() {
    ...
    setupConfigurationListeners(); // Setup listeners including the new reload button
    setupStep1Listeners();
    setupStep2Listeners();
    setupStep3Listeners();
    setupStep4Listeners();
...
function setupConfigurationListeners() {
    const ui = {
        ...
        forceReloadBtn: getElement('forceReloadBtn'), // Get the new button
    };

    // --- Add Listener for Force Reload Button ---
    ui.forceReloadBtn?.addEventListener('click', () => {
        logToConsole("Attempting hard refresh...", "warn");
        // true forces a reload from the server, attempting to bypass cache
...
function setupStep1Listeners() {
     const ui = {
        ...
        fetchSitemapBtn: getElement('fetchSitemapBtn'), // Listener added here
        ...
        // Sitemap UI elements needed after fetch completes
        sitemapLoadingIndicator: getElement('sitemapLoadingIndicator'),
    };
    ...
    ui.fetchSitemapBtn?.addEventListener('click', async () => {
        ...
        let parsedUrls = [];
        try {
            // Call helper function (now correctly exported)
            const result = await fetchAndParseSitemap(url); // Pass URL, let helper handle UI/state
            // The helper now updates state, so we just need to update UI if necessary
             const currentState = getState(); // Get updated state
             displaySitemapUrlsUI(currentState.sitemapUrls || []); // Update UI with state data

        } catch (error) {
            // Error handled/logged within fetchAndParseSitemap, just ensure UI is reset
            logToConsole(`Sitemap fetch listener caught error: ${error.message}`, 'error'); // Optional extra log
            displaySitemapUrlsUI([]); // Ensure UI is cleared on error
        } finally {
...
// --- setupStep2Listeners, setupStep3Listeners, setupStep4Listeners, setupBulkModeListeners ---
// --- (Identical to v7.1) ---
function setupStep2Listeners() { const ui = { toggleStructureVisibilityBtn: getElement('toggleStructureVisibilityBtn'), linkTypeToggle: getElement('linkTypeToggle'), linkTypeText: getElement('linkTypeText'), generateArticleBtn: getElement('generateArticleBtn'), articleTitleInput: getElement('articleTitle'), }; ui.toggleStructureVisibilityBtn?.addEventListener('click', () => { const structureContainer = getElement('structureContainer'); const isHidden = structureContainer.classList.toggle('hidden'); ui.toggleStructureVisibilityBtn.textContent = isHidden ? 'Show' : 'Hide'; }); ui.linkTypeToggle?.addEventListener('change', (e) => { const isInternal = !e.target.checked; updateState({ linkTypeInternal: isInternal }); ui.linkTypeText.textContent = isInternal ? 'Internal' : 'External'; }); ui.articleTitleInput?.addEventListener('change', (e) => { updateState({ articleTitle: e.target.value }); }); ui.generateArticleBtn?.addEventListener('click', handleGenerateArticle); }
function setupStep3Listeners() { const ui = { previewHtmlCheckbox: getElement('preview_html_checkbox'), generatedArticleTextarea: getElement('generated_article'), htmlPreviewDiv: getElement('html_preview'), enableSpinningBtn: getElement('enableSpinningBtn'), }; ui.previewHtmlCheckbox?.addEventListener('change', (e) => { const showPreview = e.target.checked; showElement(ui.generatedArticleTextarea, !showPreview); showElement(ui.htmlPreviewDiv, showPreview); if (showPreview) { let unsafeHTML = ui.generatedArticleTextarea.value; let sanitizedHTML = unsafeHTML.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, ''); ui.htmlPreviewDiv.innerHTML = sanitizedHTML; logToConsole("Showing HTML preview.", "warn"); } }); ui.enableSpinningBtn?.addEventListener('click', () => { const spunArticleDisplay = getElement('spun_article_display'); const step4Section = getElement('step4Section'); spunArticleDisplay.innerHTML = ui.generatedArticleTextarea.value; showElement(step4Section, true); highlightSpintax(spunArticleDisplay); logToConsole("Spinning enabled.", 'info'); }); }
function setupStep4Listeners() { const ui = { spunArticleDisplay: getElement('spun_article_display'), spinSelectedBtn: getElement('spinSelectedBtn'), }; ui.spunArticleDisplay?.addEventListener('mouseup', handleSelection); ui.spunArticleDisplay?.addEventListener('keyup', handleSelection); ui.spunArticleDisplay?.addEventListener('focus', handleSelection); ui.spinSelectedBtn?.addEventListener('click', handleSpinSelectedText); }
function setupBulkModeListeners() { const ui = { startBulkGenerationBtn: getElement('startBulkGenerationBtn'), downloadBulkZipBtn: getElement('downloadBulkZipBtn'), planningTableBody: getElement('planningTableBody'), }; ui.startBulkGenerationBtn?.addEventListener('click', handleStartBulkGeneration); ui.downloadBulkZipBtn?.addEventListener('click', handleDownloadZip); ui.planningTableBody?.addEventListener('change', (e) => { if (e.target.tagName === 'INPUT' && e.target.dataset.field) { const rowIndex = e.target.closest('tr')?.dataset.index; const field = e.target.dataset.field; const value = e.target.value; if (rowIndex !== undefined && field) { updateBulkPlanItem(parseInt(rowIndex, 10), { [field]: value }); logToConsole(`Updated plan item ${rowIndex}, field ${field}.`, 'info'); } } }); }
