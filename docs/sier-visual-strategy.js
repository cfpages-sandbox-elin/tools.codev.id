...
// VERSI FINAL: Merender Analisis BEP dengan rincian lengkap dan interaktif.

const sierVisualStrategy = {

    // Helper untuk membuat rincian biaya variabel (bottom-up)
    ...
    // BARU: Helper utama untuk membuat seluruh bagian BEP yang detail
    _createBEPSection(unitName) {
        ...
        // Bagian 3: Target BEP
        const bepResultsHtml = `
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <h4 class="font-semibold text-blue-800 mb-3">3. Target Break-Even Point</h4>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                    <div>
                        <p class="text-sm text-gray-600">BEP Bulanan</p>
                        <p class="text-2xl font-bold text-blue-700">${sierHelpers.formatNumber(Math.ceil(bepData.bepInUnitsMonthly))}</p>
                        <p class="text-xs text-gray-500">${bepData.unitLabel}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Target Harian (Total)</p>
                        <p class="text-2xl font-bold text-blue-700">${Math.ceil(bepData.bepInUnitsDaily)}</p>
                        <p class="text-xs text-gray-500">${bepData.unitLabel} / Hari</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Target per ${assetLabel}</p>
                        <p class="text-2xl font-bold text-blue-700">${bepData.bepPerAssetDaily.toFixed(1)}</p>
                        <p class="text-xs text-gray-500">${bepData.unitLabel} / Hari</p>
                    </div>
                </div>
            </div>
        ...
        const title = (unitName === 'drivingRange')
            ? `Model Break-Even Point (BEP) - Driving Range`
            : `Model Break-Even Point (BEP) - Studi Kasus ${courts} Lapangan`;

        return `
            <h3 class="text-xl font-semibold mb-4 pt-6 border-t mt-8 text-gray-700">${title}</h3>
            <div class="space-y-6">
                ${fixedCostsHtml}
                ${contributionMarginHtml}
                ${bepResultsHtml}
            </div>
        `;
    },

    // Fungsi render utama sekarang hanya memanggil helper
    _renderDrivingRange() {
        const container = document.getElementById('driving-range-business-strategy');
        if (!container) return;
        // Salin konten statis (non-BEP) ke sini
        const staticContent = `
