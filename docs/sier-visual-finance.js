        ...
        const createTableRows = (data, basePath) => {
            return Object.entries(data).map(([key, value]) => {
                ...
                if (typeof value !== 'object' || value === null || ('count' in value && 'salary' in value) || ('electricity_kwh_price' in value)) {
                    ...
                    return `<tr class="border-b border-gray-200 hover:bg-gray-50"><td class="py-3 px-4 text-gray-600 w-2/5">${translatedKey}</td><td class="py-3 px-4 w-3/5">${editControl}</td></tr>`;
                } else {
                    let subRows = createTableRows(value, currentPath);
                    return `<tr class="border-b border-gray-200"><td class="py-3 px-4 text-gray-600 w-2/5 align-top font-medium">${translatedKey}</td><td class="py-3 px-4 w-3/5"><table class="w-full">${subRows}</table></td></tr>`;
                }
            }).join('');
        };
