...
const sierVisualFinance = {
    ...
    _renderFinancialSummaryVisuals() {
        ...
        // Ambil total CapEx Fasilitas Umum (yang dihitung oleh fungsi _renderSharedCapexVisuals)
        const sharedCapexTotal = projectConfig.shared_facilities_capex.total || 0;

        // LANGKAH 2: Hitung ulang Total Investasi Proyek yang sebenarnya
        const totalProjectInvestment = dr.capex.total + padel.capex.total + digitalCapexTotal + sharedCapexTotal;

        // LANGKAH 3: Hitung ulang metrik kelayakan investasi menggunakan Total Investasi yang BENAR
        ...
        for (let i = 1; i <= 20; i++) { // Asumsi umur proyek 20 tahun
            npv += cashFlow / Math.pow(1 + projectConfig.assumptions.discount_rate_wacc, i);
        }

        // Hitung ulang IRR (metode iteratif sederhana)
        let irr = 0.0;
        for (let i = 0; i < 1.0; i += 0.001) { // Mencari IRR dari 0% hingga 100%
            let tempNpv = -totalProjectInvestment;
            for (let j = 1; j <= 20; j++) {
        ...
        // LANGKAH 4: Siapkan blok HTML untuk setiap bagian
        
        // Bagian 4a: Ringkasan Biaya Investasi (CapEx) dengan 5 kartu
            ...
            <div id="capex-summary" class="mb-8 pb-6 border-b">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div class="bg-gray-100 p-4 rounded-lg text-center">
                     ...
                     <div class="bg-gray-100 p-4 rounded-lg text-center">
                        <p class="text-sm text-gray-600">CapEx Fasilitas Umum</p>
                        <p class="text-2xl font-bold font-mono text-gray-800">${sierHelpers.toBillion(sharedCapexTotal)}</p>
                    </div>
                    <div class="bg-teal-600 text-white p-4 rounded-lg text-center flex flex-col justify-center">
                        ...
                        <p class="text-3xl font-bold font-mono">${sierHelpers.toBillion(totalProjectInvestment)}</p>
                    </div>
                </div>
            </div>`;
        
        // Panggil fungsi render detail depresiasi di sini, setelah semua Capex diketahui
        this._renderDepreciationDetails();

        // Bagian 4b: Proyeksi Laba Rugi Gabungan
        const createPnlTable = (data) => `<table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left"><th class="px-4 py-2 w-4/5">Deskripsi</th><th class="px-4 py-2 text-right">Nilai (Tahun Pertama)</th></tr></thead><tbody class="divide-y"><tr><td class="px-4 py-2">Pendapatan (Revenue)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.annualRevenue))}</td></tr><tr><td class="px-4 py-2">Harga Pokok Penjualan (COGS)</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualCogs))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">Laba Kotor (Gross Profit)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.grossProfit))}</td></tr><tr><td class="px-4 py-2">Biaya Operasional (OpEx)</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualOpex))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">EBITDA</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.ebitda))}</td></tr><tr><td class="px-4 py-2">Depresiasi & Amortisasi</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualDepreciation))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">Laba Sebelum Pajak (EBT)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.ebt))}</td></tr><tr><td class="px-4 py-2">Pajak Penghasilan (${(projectConfig.assumptions.tax_rate_profit * 100)}%)</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.tax))})</td></tr><tr class="bg-teal-100 font-bold text-teal-800"><td class="px-4 py-3 text-base">Laba Bersih (Net Profit)</td><td class="px-4 py-3 text-right text-base font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.netProfit))}</td></tr></tbody></table>`;
        const pnlSummaryHtml = `
            <div id="pnl-summary" class="mb-8 pb-6 border-b">
                <h3 class="text-xl font-semibold mb-3 text-gray-800">2. Proyeksi Laba Rugi (P&L) - Gabungan</h3>
                <div class="overflow-x-auto border rounded-lg">${createPnlTable(combined)}</div>
            </div>`;
        
        // Bagian 4c: Analisis Kelayakan Investasi dengan metrik yang sudah dihitung ulang
        const investmentFeasibilityHtml = `
            <div id="investment-feasibility" class="mb-8 pb-6 border-b">
                <h3 class="text-xl font-semibold mb-4 text-gray-800">3. Analisis Kelayakan Investasi (Proyek Gabungan)</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-blue-100 p-4 rounded-lg text-center">
                        <p class="text-sm text-blue-800">Payback Period</p>
                        <p class="text-3xl font-bold text-blue-700">${paybackPeriod.toFixed(2)}</p>
                        <p class="text-xs text-blue-600">Tahun</p>
                    </div>
                    <div class="bg-green-100 p-4 rounded-lg text-center">
                        <p class="text-sm text-green-800">Net Present Value (NPV)</p>
                        <p class="text-3xl font-bold text-green-700">${sierHelpers.toBillion(npv)}</p>
                        <p class="text-xs text-green-600">(WACC ${projectConfig.assumptions.discount_rate_wacc * 100}%)</p>
                    </div>
                    <div class="bg-purple-100 p-4 rounded-lg text-center">
                        <p class="text-sm text-purple-800">Internal Rate of Return (IRR)</p>
                        <p class="text-3xl font-bold text-purple-700">${(irr * 100).toFixed(2)}%</p>
                        <p class="text-xs text-purple-600">Lebih besar dari WACC, proyek layak</p>
                    </div>
                </div>
            </div>`;
            
        // Bagian 4d: Analisis Sensitivitas (Tetap sama, tidak terpengaruh langsung)
        const sensitivityAnalysisHtml = `
            <div id="sensitivity-analysis">
                <h3 class="text-xl font-semibold mb-4 text-gray-800">4. Analisis Sensitivitas</h3>
                <p class="text-gray-600 mb-4 text-sm">Analisis ini menguji seberapa besar perubahan Laba Bersih Tahunan jika asumsi pendapatan atau biaya operasional berubah. Ini membantu mengidentifikasi risiko utama terhadap profitabilitas.</p>
                <!-- Konten visual untuk analisis sensitivitas dapat ditambahkan di sini, misal: tabel atau chart -->
            </div>`;

        // LANGKAH 5: Gabungkan semua blok HTML dan render ke kontainer utama
        summaryContainer.innerHTML = `
            ...
            <div class="bg-white p-6 rounded-lg shadow-md mb-8">
                <div class="p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 mb-6 text-sm">
                    <strong>Disclaimer:</strong> Analisis ini adalah model proyeksi berdasarkan serangkaian asumsi. Angka aktual dapat bervariasi.
                </div>
                ${capexSummaryHtml}
                <!-- Rincian Depresiasi akan dirender oleh fungsinya sendiri ke dalam div ini -->
                <div id="depreciation-details-container" class="mb-8 pb-6 border-b"></div>
                ${pnlSummaryHtml}
                ${investmentFeasibilityHtml}
                ${sensitivityAnalysisHtml}
            </div>
        `;
        
        // Panggil render depresiasi SETELAH kontainer targetnya ada di DOM
        this._renderDepreciationDetails();

        console.log("[sier-visual-finance] Ringkasan Keuangan Gabungan: Berhasil dirender ulang dengan semua komponen CapEx.");
    },
