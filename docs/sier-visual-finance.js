...
// VERSI BARU: Merender semua elemen visual non-chart terkait Keuangan.
// SEKARANG JUGA BERTANGGUNG JAWAB ATAS RENDER DETAIL BEP, PAYBACK, & SKENARIO.
...
const sierVisualFinance = {
        ...
        const createBusinessModelSection = (title, data, basePath, theme) => {
            ...
            return `
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-4"><table class="w-full text-sm"><tbody class="divide-y divide-gray-200">${createTableRows(data, basePath)}</tbody></table></div>
                </div>
            `;
        };
    ...
    _renderCapexVisuals() {
        ...
        const scenarioTableHtml = (title, data) => {
            ...
            return `<div class="mb-10"><h3 class="text-2xl font-bold text-gray-800 mb-4">${title}</h3><p class="text-gray-600 mb-6">${data.description}</p><div class="overflow-x-auto border rounded-lg"><table class="w-full text-sm"><thead class="bg-gray-100"><tr><th class="p-3 text-left w-2/5">Komponen</th><th class="p-3 text-left w-2/5">Asumsi</th><th class="p-3 text-right w-1/5">Biaya (Rp)</th></tr></thead><tbody class="divide-y">${data.htmlRows.map(r => createRow(r.label, r.calculation, r.value, r.path)).join('')}<tr class="bg-gray-200 font-bold"><td class="p-3" colspan="2">Subtotal Biaya Fisik & Lainnya</td><td class="p-3 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.subtotal))}</td></tr><tr class="bg-yellow-200 font-bold"><td class="p-3" colspan="2">Kontingensi (${contingencyRate}%)</td><td class="p-3 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.contingency))}</td></tr><tr class="bg-amber-400 text-white font-bold text-lg"><td class="p-3" colspan="2">Total Estimasi Biaya</td><td class="p-3 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.total))}</td></tr></tbody></table></div></div>`;
        };
    ...
    // --- BARU: HELPER UNTUK MERENDER KARTU BEP, PAYBACK, DLL ---
    _createBEPSection(analysisData) {
        const { bepAnalysis } = analysisData;
        return `<div class="bg-gray-50 p-4 rounded-lg border"><h4 class="font-semibold text-gray-800 mb-2">A. Analisis Titik Impas Operasional (BEP)</h4><p class="text-sm text-gray-600 mb-3">Target penjualan minimum agar tidak rugi secara operasional (laba = 0).</p><div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div><table class="w-full text-sm"><tbody class="divide-y">${Object.entries(bepAnalysis.fixedCostBreakdown).map(([key, value]) => `<tr><td class="py-1 text-gray-600">${key}</td><td class="py-1 text-right font-mono">Rp ${sierHelpers.formatNumber(Math.round(value))}</td></tr>`).join('')}</tbody><tfoot class="border-t-2"><tr class="font-bold"><td class="py-1">Total Biaya Tetap Bulanan</td><td class="py-1 text-right font-mono">Rp ${sierHelpers.formatNumber(Math.round(bepAnalysis.totalFixedCostMonthly))}</td></tr></tfoot></table></div><div class="bg-white p-3 rounded-md shadow-sm text-center flex flex-col justify-center"><p class="text-sm font-semibold text-gray-700">Target BEP Bulanan</p><p class="text-4xl font-bold text-blue-700 mt-1">${sierHelpers.formatNumber(Math.ceil(bepAnalysis.bepInUnitsMonthly))}</p><p class="text-sm text-gray-500">${bepAnalysis.unitLabel}</p><p class="text-xs text-gray-500 mt-2">(~${Math.ceil(bepAnalysis.bepInUnitsDaily)} ${bepAnalysis.unitLabel} / hari)</p></div></div></div>`;
    },
    ...
    _renderFinancialSummaryVisuals() {
        ...
        // --- DIUBAH: Sekarang merender analisis detail untuk setiap unit bisnis ---
        ...
        const createPnlTable = (title, data) => `<div class="mb-8 pb-6 border-b"><h3 class="text-xl font-semibold mb-3 text-gray-800">${title}</h3><div class="overflow-x-auto border rounded-lg"><table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left"><th class="px-4 py-2 w-3/5">Deskripsi</th><th class="px-4 py-2 text-right">Nilai (Tahun Pertama)</th></tr></thead><tbody class="divide-y"><tr><td class="px-4 py-2">Pendapatan (Revenue)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.annualRevenue))}</td></tr><tr><td class="px-4 py-2">Harga Pokok Penjualan (COGS)</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualCogs))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">Laba Kotor (Gross Profit)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.grossProfit))}</td></tr><tr><td class="px-4 py-2">Biaya Operasional (OpEx)</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualOpex))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">EBITDA</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.ebitda))}</td></tr><tr><td class="px-4 py-2">Depresiasi & Amortisasi</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.annualDepreciation))})</td></tr><tr class="font-semibold"><td class="px-4 py-2">Laba Sebelum Pajak (EBT)</td><td class="px-4 py-2 text-right font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.ebt))}</td></tr><tr><td class="px-4 py-2">Pajak Penghasilan</td><td class="px-4 py-2 text-right font-mono">(${sierHelpers.formatNumber(Math.round(data.pnl.tax))})</td></tr><tr class="bg-teal-100 font-bold text-teal-800"><td class="px-4 py-3 text-base">Laba Bersih (Net Profit)</td><td class="px-4 py-3 text-right text-base font-mono">${sierHelpers.formatNumber(Math.round(data.pnl.netProfit))}</td></tr></tbody></table></div></div>`;

        // Render setiap bagian Driving Range
        drContainer.innerHTML = `
            <h2 class="text-2xl font-semibold mb-6 text-gray-800 border-l-4 border-amber-500 pl-4">Analisis Finansial Detail: Driving Range</h2>
            <div class="bg-white p-6 rounded-lg shadow-md mb-8 space-y-8">
                ${createPnlTable('Proyeksi Laba Rugi - Driving Range', dr)}
                ${this._createBEPSection(drAnalysisData)}
                ${this._createProfitabilitySection(drAnalysisData)}
                ${this._createScenarioSection(drAnalysisData)}
            </div>
        `;
        
        // Render setiap bagian Padel
        padelContainer.innerHTML = `
            <h2 class="text-2xl font-semibold mb-6 text-gray-800 border-l-4 border-purple-600 pl-4">Analisis Finansial Detail: Padel</h2>
            <div class="bg-white p-6 rounded-lg shadow-md mb-8 space-y-8">
                ${createPnlTable('Proyeksi Laba Rugi - Padel', padel)}
                ${this._createBEPSection(padelAnalysisData)}
                ${this._createProfitabilitySection(padelAnalysisData)}
                ${this._createScenarioSection(padelAnalysisData)}
            </div>
        `;
        
        // Render bagian Gabungan (Combined) yang sudah ada
        summaryContainer.innerHTML = `<h2 class="text-2xl font-semibold mb-6 text-gray-800 border-l-4 border-teal-600 pl-4">Analisis Keuangan & Kelayakan Investasi (Proyek Gabungan)</h2><div class="bg-white p-6 rounded-lg shadow-md mb-8"><div class="mb-8 pb-6 border-b"><h3 class="text-xl font-semibold mb-3 text-gray-800">Ringkasan Biaya Investasi (CapEx)</h3><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="bg-gray-100 p-4 rounded-lg text-center"><p class="text-sm text-gray-600">CapEx Driving Range</p><p class="text-2xl font-bold font-mono text-gray-800">${sierHelpers.toBillion(dr.capex.total)}</p></div><div class="bg-gray-100 p-4 rounded-lg text-center"><p class="text-sm text-gray-600">CapEx Padel</p><p class="text-2xl font-bold font-mono text-gray-800">${sierHelpers.toBillion(padel.capex.total)}</p></div><div class="bg-teal-600 text-white p-4 rounded-lg text-center"><p class="text-sm">Total Investasi Proyek</p><p class="text-2xl font-bold font-mono">${sierHelpers.toBillion(combined.capex.total)}</p></div></div></div>${createPnlTable('Proyeksi Laba Rugi - Proyek Gabungan', combined)}<div id="investment-feasibility" class="mb-8 pb-6 border-b"><h3 class="text-xl font-semibold mb-4 text-gray-800">Analisis Kelayakan Investasi</h3><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="bg-blue-100 p-4 rounded-lg text-center"><p class="text-sm text-blue-800">Payback Period</p><p class="text-3xl font-bold text-blue-700">${combined.feasibility.paybackPeriod.toFixed(2)}</p><p class="text-xs text-blue-600">Tahun</p></div><div class="bg-green-100 p-4 rounded-lg text-center"><p class="text-sm text-green-800">Net Present Value (NPV)</p><p class="text-3xl font-bold text-green-700">${sierHelpers.toBillion(combined.feasibility.npv)}</p><p class="text-xs text-green-600">(WACC ${projectConfig.assumptions.discount_rate_wacc * 100}%)</p></div><div class="bg-purple-100 p-4 rounded-lg text-center"><p class="text-sm text-purple-800">Internal Rate of Return (IRR)</p><p class="text-3xl font-bold text-purple-700">${(combined.feasibility.irr * 100).toFixed(2)}%</p><p class="text-xs text-purple-600">per Tahun</p></div></div></div></div>`;
    },

    // Fungsi render utama memanggil semua sub-render
    render() {
