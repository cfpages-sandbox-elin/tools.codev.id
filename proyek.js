document.addEventListener('DOMContentLoaded', () => {
    ...
    let projectData = {};
    let currentView = 'kontraktor';

    const setupTabs = () => {
        tabKontraktor.addEventListener('click', () => {
            currentView = 'kontraktor';
    ...
    const calculateProject = () => {
        ...
        // --- 2. KALKULASI UTAMA (BERDASARKAN RUMUS DI JSON) ---

        // A. Durasi & Biaya Tenaga Kerja
        ...
        const totalBiayaTukang = biayaHarianTukang * 25 * durasiBulan; // Asumsi 25 hari kerja/bulan
        ...
        spesifikasi.forEach(item => {
            ...
            // Buat baris tabel untuk Tampilan Kontraktor
            rabKontraktorHTML += `
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${item.nama}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${volume.toFixed(2)} ${item.satuan}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${formatRupiah(item.harga.kulak)}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-semibold">${formatRupiah(subtotalKulak)}</td>
                </tr>
            `;
            
            // Buat baris tabel untuk Tampilan Klien
            rabKlienHTML += `
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${item.nama}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${volume.toFixed(2)} ${item.satuan}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${formatRupiah(item.harga.pasar)}</td>
        ...
        const profitMargin = (profitEstimasi / totalNilaiProyekKlien) * 100;
        ...
        // B. Render Tabel RAB (termasuk total)
        // Asumsi di HTML ada <tbody id="output-rab-table-kontraktor"> dan <tbody id="output-rab-table-klien">
        ...
        // C. Render Timeline & Termin Dinamis
        let timelineHTML = '';
        ...
        milestones.forEach(milestone => {
            ...
            if (milestone.persentase > terminBatasAtas && currentTermin < jumlahTermin) {
                terminHTML_Content += `</ul></div>`; // Tutup termin sebelumnya
                currentTermin++;
        ...
        terminHTML_Content += `</ul></div>`; // Tutup termin terakhir
        // Tambahkan Retensi
